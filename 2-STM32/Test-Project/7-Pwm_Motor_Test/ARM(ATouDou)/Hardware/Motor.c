...
uint32_t set_speed  = 240; 		// 最大速度,	为speed*0.1*rad/sec
uint32_t step_accel = 100; 		// 加速度,		为decel*0.1*rad/sec^2
uint32_t step_decel = 100; 		// 减速度,		为accel*0.1*rad/sec^2
...
void TIM8_CC_IRQHandler(void) 
{
    ...
    if(TIM_GetITStatus(TIM8, TIM_IT_CC3) == SET) TIM_ClearITPendingBit(TIM8, TIM_IT_CC3);
    if(TIM_GetITStatus(TIM8, TIM_IT_CC4) == SET) TIM_ClearITPendingBit(TIM8, TIM_IT_CC4);
}
